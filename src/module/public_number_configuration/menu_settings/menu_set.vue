...
<template>
	<div class="mall-menu" style="width: 1517px;" id="menu" v-cloak>
		...
		<!-- 新建分组菜单 -->
		<groupingMenu v-if="isShowMenuWin" :menuselName="menuselName" @getGroupingResult="getGroupingResult"></groupingMenu>
		<!-- 快速创建菜单 -->
...
export default {
	...
	methods: {
		...
		getGroupingResult(str, selectId) { //新建分组菜单的弹窗回调
			if (str == 'ok') {
				for (let key of this.menuName) {
					if (key.matchRule == selectId) {
	...
	components: {
		...
		groupingMenu: () =>
			import( /* webpackChunkName:"grouping_menu_win" */ './grouping_menu_win'),
	}
};
</script>
