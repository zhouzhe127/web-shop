...
<template>
	<div class="mall-menu" style="width: 1517px;" id="menu" v-cloak>
		<header class="menu_header">
			<ul>
				<li>
					<button @click="selectMenu('default',0)" :class="menuIndex == 0?'btn_blue':''" class="btns">默认菜单</button>
				</li>
				<li v-for="(item,index) in menuName" :key="index">
					<button @click="selectMenu(item,index+1)" :class="index+1 == menuIndex?'btn_blue':''" class="btns">{{item.name}}</button>
				</li>
			</ul>
		</header>
