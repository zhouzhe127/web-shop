...
<template>
	<div id='rushedRecord'>
		...
		<div class="list_box" style="width:100%;">
			...
			<el-table :data='formList' border :header-row-style="{'height':'40px'}" :stripe='true' :header-cell-style="{'background-color':'#f5f7fa'}" :row-style="{'height':'70px'}">
				<el-table-column fixed prop='shopId' label='店铺名称' align='center'>
					<template slot-scope='scope'>
						<span>{{getshopName(scope.row.shopId)}}</span>
					</template>
				</el-table-column>
				<el-table-column prop='stock' label='原始库存' :render-header='renderHeader' show-overflow-tooltip align='center'>
				</el-table-column>
				<el-table-column prop='grabNum' label='抢购数量' :render-header='renderHeader' show-overflow-tooltip align='center'>
				</el-table-column>
				<el-table-column prop='receiveNum' label='已领取数量' :render-header='renderHeader' show-overflow-tooltip align='center'>
				</el-table-column>
