<template>
	<div id="worker_staff">
		<template v-if="workerType == 'worker'">
			...
			<com-table :listWidth="1470" :listHeight='80' :listName="'工作人员'" :key="index" :showTitle='1' :introData="userList" :titleData="titleList" :allTotal="wokerList.length" :widthType='true'>
				<div slot="con-0" slot-scope="props" class="operate_worker">
					<span @click="editUser(props.data,props.index)">编辑</span>
					<span @click="deleteUser(props.data,props.index)">删除</span>
				</div>
				<div slot="con-1" slot-scope="props">{{(page-1) * 10 + props.index + 1}}</div>
				<div class="imgStyle" slot="con-3" slot-scope="props">
					<img :src="props.data.imageUrl" :alt="props.data.name" />
				</div>
				<div slot="con-5" slot-scope="props" @click='openCustomer(props.data)'>{{props.data.customerNum}}</div>
				<div slot="con-6" slot-scope="props" @click='openCoin(props.data)'>{{props.data.coins}}</div>
				<div slot="con-9" slot-scope="props">{{statusType[props.data.position]}}</div>
			</com-table>
			<!-- 翻页 -->
