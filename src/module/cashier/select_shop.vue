<template>
	<div class="detLiBox">
		<div @click="shop" class="select-body-btn select-btn">
			<span ref="spanBtn">
				<i v-if="showNum && selectedShop != '请选择店铺'">已选择{{selectedShop.split(',').length}}家门店</i>
				<i v-else>{{selectedShop}}</i>
			</span>
			<em>
				<i></i>
			</em>
		</div>
		<div v-if="showShop" @click.stop class="detDivBox" style="width: 500px;top: 52px;left: -10px;">
	...
	export default {
		...
		methods: {
			...
			//确定
				...
				this.$emit('returnShop', this.shopIds, this.selectedShop,this.shopList);
			},
			//取消
			cancel() {
				this.showShop = false;
...
<style scoped>
	...
	.detLiBox .detDivBox ul li.active {
	...
	.detLiBox .jobBtn .selectbtn {
		color: #FF9200;
		border-bottom: 4px solid #FF9200;
	}

	.detLiBox .detLiBtn span {
		float: left;
		color: #fff;
		width: 50%;
		height: 40px;
