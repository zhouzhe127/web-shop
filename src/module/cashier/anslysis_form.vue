 ...
 * @Last Modified time: 2018-08-13 10:45:55 
 * @Module: 收银分析表格
 */
		...
		methods: {
			...
			exportData(){
				...
				http.exportAnalysisReport({
					data: {
						shopId: this.shopId,
						analysisInfo: JSON.stringify(exportData),
			...
			selected(item, index) {
				if (index !== undefined) {
					item.selected ? this.orderPageList[index].selected = false : this.orderPageList[index].selected = true;
					for (let i = 0; i < this.orderPageList.length; i++) {
						for (let j = 0; j < this.orderList.length; j++) {
							if (this.orderList[j].time == this.orderPageList[i].time) {
								this.orderList[j].selected = this.orderPageList[i].selected;
							}
						}
					}
				} else {
