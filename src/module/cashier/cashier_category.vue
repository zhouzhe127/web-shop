...
<template>
	<div id="report-cate">
		...
		<categoryRetail v-if="orderList.length > 0 && loading" :taskId="taskId" :chooseShopList="chooseShopList.filter(v=>v.selected)" :shopList="shopList" :orderList="orderList" :loading="loading"
		    :selShopid="selShopid" :shopName="shopName" :selectedType="selectedType"></categoryRetail>
			...
			async taskInfo(data) {
				http.taskInfo({
					...
					if (res.status == 3) {
						...
						this.repeat = false;
						this.loading = true;
						this.$store.commit('setWin', {title: '温馨提示',winType: 'alter',content: '查询失败，请重试'});
					}
				});
			},
			// 轮询结束获取数据
...
//			},
			...
			search() {
				if(this.endTime < this.startTime){
					this.$store.commit('setWin', {title: '温馨提示',winType: 'alert',content: '结束时间不能小于开始时间',});
					return false;
				}
				if (this.repeat) {
					this.$store.commit('setWin', {title: '温馨提示',winType: 'alert',content: '当前查询尚未结束，请结束后再进行查询或刷新浏览器重新查询！',});
					return false;
				}
				if (this.selShopid.length == 0) {
					this.$store.commit('setWin', {title: '温馨提示',winType: 'alter',content: '请选择店铺'});
					return false;
				}
				this.loading = false;
				this.repeat = true;
