	...
	<div id="report_category">
		...
		<section>
			<section>
				...
				<div class="mbot">
					<span class="sel" v-for="(item, index) in dataList.list" :key="index" v-bind:class="{'on': dataList.buttonTop == index}"
			...
			<section v-show="chartList.flag === 0">
								...
								<div class="aLeft" :style="{'width': (headList.length*100+200*1  - 59) + 'px'}">
									<section class="orMaRetreatMain">
										...
										<ul class="orMaReMainList listLeft rightTotal">
											<li :style="{'width': (headList.length*100+200*1-60) + 'px'}" class="chairFix" style="background: #f7f7f7;">
				...
				<page v-if="total > 1" @pageNum="pageChange" :page.sync="page" :total.sync='total' :isNoJump='true' :isNoPaging='true'
					style="float: left;margin-bottom: 100px;"></page>
			</section>
		</section>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			initBtn() {
				let arr = [{
					...
					fn: () => {
						...
						for(let i in data){ //一级分类
							if(!data[i].click){ //若一级分类没有展开 则二级分类和菜品置为空
								data[i].q = {};
								data[i].r = {};
							}
							let child = data[i].q; //二级分类
							for(let j in child){
								if(!child[j].click){ //若二级分类没有展开，则二级分类菜品置为空
									child[j].r = {};
								}
			...
			dataHandle(){
				...
				let obj = {}, brandId = -1; //总计数据obj 存只要在门店出现的分类和商品，将分类和商品以键值对的形式保存
				// let oldData = {goodsNum: 0,num: 0,totalPrice: 0,attrPrice: 0,freeNum: 0,freePrice: 0,discountPrice: 0,price: 0,};
				let oldData = {c: 0,d: 0,l: 0,e: 0,f: 0,g: 0,j: 0,k: 0,h: 0,i: 0};
				// let dayObj = {};
				for(let i = 0; i < data.length; i++){
					...
					for(let j in shopList){
						...
						for(let k = 0; k <oneCate.length; k++){
							...
							oneCate[k].u < 100000 ? oneKey = `${oneCate[k].u}_${brandId}` : oneKey = `${oneCate[k].u}_${oneCate[k].a}`;
							if(!oneHash[oneKey]){ //若一级分类哈希中没有匹配到改门店分类，则将该门店分类存入总计数据obj中
								obj[oneKey] = utils.deepCopy(oneCate[k]);
								for(let key in oldData){
									obj[oneKey][key] = 0; //将总计原始数值设为0
								}
								if(oneCate[k].u >= 100000) obj[oneKey].t += `(${oneCate[k].b})`;
								obj[oneKey].q = {};
								obj[oneKey].r = {};
								oneHash[oneKey] = true;
							}
							if(!totalHash[oneKey]){
								...
								if(oneCate[k].u >= 100000) dayObj[oneKey].t += `(${oneCate[k].b})`;
								dayObj[oneKey].q = {};
								dayObj[oneKey].r = {};
								totalHash[oneKey] = true;
							}
							for(let key in oldData){ //将每家店铺的一级分类数据相加放入总计数据中
			...
			pageChange: function (item) {
				let page = item.page;
				this.orderPageList = this.classification.slice((page - 1) * this.num, (page - 1) * this.num + this.num);
				//计算总宽度
				this.allShopIndex = 0;
			...
			getData(...argus){  //获取一级分类下的菜品、二级分类   获取二级分类下的菜品
				let [data,type,categoryId,categoryKey,childId,childKey] = argus;
				let oneHash = {},twoHash = {}, threeHash = {}; //各个分类和商品的哈希，匹配各个门店，有这个值则跳过
				let brandId = -1;
				let obj;
				if(type == 'category'){ //保存所有合计
					obj = {r:{},q:{}};
				}else{
					obj = {};
				}
				...
				for(let i = 0; i < data.length; i++){
					let shopList = data[i].shop;
					let dayObj,totalOneHash = {},totalTwoHash = {};
					if(type == 'category'){ //保存每一天的合计
						dayObj = {r:{},q:{}};
					}else{
						dayObj = {};
					}
					for(let j in shopList){
						...
						for(let k = 0; k < oneCate.length; k++){
							if(type == 'category'){
								let oneKey; //一级分类的键，若品牌分类则为分类id加品牌id；若为门店分类则为分类id加门店id
								if(oneCate[k].u){ //一级分类下的二级分类
									oneCate[k].u < 100000 ? oneKey = `${categoryId}_${oneCate[k].u}_${brandId}` : oneKey = `${categoryId}_${oneCate[k].u}_${oneCate[k].a}`;
									if(!oneHash[oneKey]){ //若一级分类哈希中没有匹配到改门店分类，则将该门店分类存入总计数据obj中
										obj.q[oneKey] = utils.deepCopy(oneCate[k]);
										for(let key in oldData){
											obj.q[oneKey][key] = 0; //将总计原始数值设为0
										}
										if(oneCate[k].u >= 100000) obj.q[oneKey].t += `(${oneCate[k].b})`; //若为门店自建 在分类名后加上门店名称
										obj.q[oneKey].q = {};
										obj.q[oneKey].r = {};
										oneHash[oneKey] = true;
									}
									if(!totalOneHash[oneKey]){ //计算每一天的合计数据
										dayObj.q[oneKey] = utils.deepCopy(oneCate[k]);
										for(let key in oldData){
											dayObj.q[oneKey][key] = 0; //将总计原始数值设为0
										}
										if(oneCate[k].u >= 100000) dayObj.q[oneKey].t += `(${oneCate[k].b})`; //若为门店自建 在分类名后加上门店名称
										dayObj.q[oneKey].q = {};
										dayObj.q[oneKey].r = {};
									...
									for(let key in oldData){ //将每家店铺的一级分类数据相加放入总计数据中
										oneCate[k][key] = Number(oneCate[k][key]);
										obj.q[oneKey][key] += Number(oneCate[k][key]);
										dayObj.q[oneKey][key] += Number(oneCate[k][key]);
									}
									dayObj.q[oneKey]['a'] = '-1';
								}else{ //一级分类下的商品
									oneCate[k].v < 100000 ? oneKey = `${categoryId}_${oneCate[k].v}_${brandId}` : oneKey = `${categoryId}_${oneCate[k].v}_${oneCate[k].a}`;
									if(!twoHash[oneKey]){ //若一级分类哈希中没有匹配到改门店分类，则将该门店分类存入总计数据obj中
										obj.r[oneKey] = utils.deepCopy(oneCate[k]);
										for(let key in oldData){
											obj.r[oneKey][key] = 0; //将总计原始数值设为0
										}
										if(oneCate[k].v >= 100000) obj.r[oneKey].w += `(${oneCate[k].b})`; //若为门店自建 在分类名后加上门店名称
										obj.r[oneKey].q = {};
										obj.r[oneKey].r = {};
										twoHash[oneKey] = true;
									}
									if(!totalTwoHash[oneKey]){ //若一级分类哈希中没有匹配到改门店分类，则将该门店分类存入总计数据obj中
										dayObj.r[oneKey] = utils.deepCopy(oneCate[k]);
										for(let key in oldData){
											dayObj.r[oneKey][key] = 0; //将总计原始数值设为0
										}
										if(oneCate[k].v >= 100000) dayObj.r[oneKey].w += `(${oneCate[k].b})`; //若为门店自建 在分类名后加上门店名称
										dayObj.r[oneKey].q = {};
										dayObj.r[oneKey].r = {};
									...
									for(let key in oldData){ //将每家店铺的一级分类数据相加放入总计数据中
										oneCate[k][key] = Number(oneCate[k][key]);
										obj.r[oneKey][key] += Number(oneCate[k][key]);
										dayObj.r[oneKey][key] += Number(oneCate[k][key]);
									}
									dayObj.r[oneKey]['a'] = -1;
								}
							}else if(type == 'child'){
								let goodsKey;//二级分类下的菜品的键
								oneCate[k].v < 100000 ? goodsKey = `${categoryId}_${childId}_${oneCate[k].v}_${brandId}` : goodsKey = `${categoryId}_${childId}_${oneCate[k].v}_${oneCate[k].a}`;
								if(!threeHash[goodsKey]){//匹配总计中二级分类是否含有该商品
								...
								if(!totalOneHash[goodsKey]){//匹配总计中二级分类是否含有该商品
									dayObj[goodsKey] = utils.deepCopy(oneCate[k]);
									for(let key in oldData){
										dayObj[goodsKey][key] = 0;
									}
									if(oneCate[k].v >= 100000) dayObj[goodsKey].w += `(${oneCate[k].b})`;
									totalOneHash[goodsKey] = true;
								}
								for(let key in oldData){//将每家店铺的二级分类下的菜品相加
									oneCate[k][key] = Number(oneCate[k][key]);
									obj[goodsKey][key] += Number(oneCate[k][key]);
									dayObj[goodsKey][key] += Number(oneCate[k][key]);
								}
								dayObj[goodsKey]['a'] = -1;
							}
						}
					}
					for(let k = 0; k < this.classification.length; k++){ //为每一天加入店铺合计 键为'-1'表示为总计数据
				...
				for(let i = 0; i < this.classification.length; i++){  
					...
					for(let j in shopList){
						let cate = shopList[j];
						if(type == 'category'){// 一级分类下的二级分类和一级分类下的菜品
							for(let m = 0; m < data.length; m++){ 
								if(data[m].time == this.classification[i].time){//与店铺每一天数据进行匹配，将数据插入到匹配的当天
									for(let n in data[m].shop){
										let oneCate = data[m].shop[n];
										if(cate.hasOwnProperty(categoryKey) && n != '-1' && oneCate[0].a == j){ //将每家店的二级分类转成键值对形式
											for(let k in oneCate){
												let oneKey;//设置二级分类或菜品的键，用于匹配渲染
												if(oneCate[k].u){ //二级分类
													oneCate[k].u < 100000 ? oneKey = `${categoryId}_${oneCate[k].u}_${brandId}` : oneKey = `${categoryId}_${oneCate[k].u}_${oneCate[k].a}`;
													cate[categoryKey].q[oneKey] = data[m].shop[n][k];
												}else{ // 一级分类下的菜品
													oneCate[k].v < 100000 ? oneKey = `${categoryId}_${oneCate[k].v}_${brandId}` : oneKey = `${categoryId}_${oneCate[k].v}_${oneCate[k].a}`;
													cate[categoryKey].r[oneKey] = data[m].shop[n][k];
												}
											}
										}
									}
							...
							if(this.formList.hasOwnProperty(categoryKey)){ //将获取的数据放入门店合计数据中
								this.formList[categoryKey].q = obj.q;
								this.formList[categoryKey].r = obj.r;
								for(let k in this.formList[categoryKey].q){
									this.$set(this.formList[categoryKey].q[k], 'click', false);
									this.$set(this.formList[categoryKey].q[k], 'loading', false);
								}
							}
						}else if(type == 'child'){ //二级分类下的菜品
							if(cate[categoryKey] && cate[categoryKey].q[childKey]){
							...
							for(let m = 0; m < data.length; m++){
								if(data[m].time == this.classification[i].time){//与店铺每一天数据进行匹配，将数据插入到匹配的当天
									for(let n in data[m].shop){
										let oneCate = data[m].shop[n];
										if(cate.hasOwnProperty(categoryKey) && cate[categoryKey].q.hasOwnProperty(childKey) && n != '-1' && oneCate[0].a == j){
											for(let k in oneCate){
												let goodsKey;//二级分类下的菜品的键
												oneCate[k].v < 100000 ? goodsKey = `${categoryId}_${childId}_${oneCate[k].v}_${brandId}` : goodsKey = `${categoryId}_${childId}_${oneCate[k].v}_${oneCate[k].a}`;
							...
							if(this.formList.hasOwnProperty(categoryKey) && this.formList[categoryKey].q.hasOwnProperty(childKey)){
			...
			getDetailsOne: function (key, item) {
				...
				item.click = !click;
				if(item.click && (!Object.keys(item.q).length && !Object.keys(item.r).length)){
					item.loading = true;
