	...
	export default {
		...
		methods:{
            ...
            async changeShopInfo(){
				...
				let currentShop = this.currentDetail ? this.currentDetail : this.userData.currentShop;
						...
						for (let i = 0; i < shopList.brand.length; i++) {
							if (this.baseDetial.brandId == shopList.brand[i].id) {
								if (this.baseDetial.ischain == '1') {
									for (let j = 0; j < shopList.brand[i].direct.length; j++) {
										if (this.baseDetial.id == shopList.brand[i].direct[j].id) {
											shopList.brand[i].direct[j] = this.baseDetial;
											shopList.brand[i].direct[j].availableState = 0;
										}
									}
								}
							}else if (this.baseDetial.ischain == '2') {
								for (let j = 0; j < shopList.brand[i].franchise.length; j++) {
									if (this.baseDetial.id == shopList.brand[i].franchise[j].id) {
										shopList.brand[i].franchise[j] = this.baseDetial;
										shopList.brand[i].franchise[j].availableState = 0;
									}
								}
							}
						}
