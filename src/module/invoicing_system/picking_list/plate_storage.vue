						...
						<ul class="oUl oulSecond" v-for="(item, index) in infoList" :key="index">
							...
							<li>
								<el-select v-model="item.index" @change="(res)=>{selectType(item,res)}" style="width:100px;">
									<el-option v-for="elItem in item.options" :key="elItem.value" :label="elItem.label" :value="elItem.value">
									</el-option>
								</el-select>
							</li>
							...
							<li style="width: 20%;">
								<div class="div_float">
									<input class="searchgoods" type="text" :onkeyup="getmin(item)" v-model="item.useNum" placeholder="请输入" :disabled="item.haveBatch" />
								...
								<template v-if="item.selUnit.isMin != 1">
									...
									<div class="div_float">
										<input class="searchgoods" type="text" :onkeyup="getmin(item)" v-model="item.useWeight" placeholder="请输入" :disabled="item.haveBatch" />
							...
							<li style="width: 20%">
								<div class="div_float">
									<input class="searchgoods" type="text" :onkeyup="getmin(item,'1')" v-model="item.backNum" placeholder="请输入" :disabled="item.haveBatch" />
									<span class="span_line" v-if="item.selUnit.name" :title="item.selUnit.name">{{item.selUnit.name}}</span>
								</div>
								<template v-if="item.selUnit.isMin != 1">
									<span class="div_float add-sign">+</span>
									<div class="div_float">
										<input class="searchgoods" type="text" :onkeyup="getmin(item,'1')" v-model="item.backWeight" placeholder="请输入" :disabled="item.haveBatch" />
										<span class="span_line" :title="item.isMin">{{item.isMin}}</span>
									</div>
								</template>
							</li>
	...
	export default {
		...
		methods: {
			...
			cancel() {
				...
				this.$router.push({
					path: '../pickingList/checkDetails',
					query: this.$route.query
				});
			},
			//领料盘库确定
			async invoicingCheckMaterial(obj) {
				let res = await http.invoicingCheckMaterial({
					data: obj
				});
				if (res == false) {
					this.$message({
			...
			enter() {
				for (let infoItem of this.infoList) {
					if (infoItem.usemin > infoItem.surplus) {
						this.$message({
							message: `物料: ${infoItem.materialName} 剩余数量不能大于领料数量`,
							type: 'error'
						});
						return false;
					}
		...
		components: {
			...
			batchwin: () =>
				import ( /*webpackChunkName: 'storage_win'*/ './storage_win')
		}
	};
</script>
