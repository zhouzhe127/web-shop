		...
		methods: {
			...
			veriConfirmClick(){
				...
				if(this.isUpdateZero && !this.selList.length && !this.storeAll){
					this.$confirm('未选中的商品库存将消耗至0，减少量日志记录为批盘消耗量（未选中任何商品，直接盘库，该操作15分钟只能生效一次）', '提示', {
			...
			//不填写数量直接提交
			checkGoodsSubmit(){
				http.GoodsinventoryBatchSetGoodsInventory({data:{
					type:1,
					data:[],
			...
			async editTemplate(){//编辑模板
				...
				this.type = data.content.type ? data.content.type:'';
				this.selectItem = data.content.items;
				this.isUpdateZero = data.content.isUpdateZero==1;
				this.goodsName = data.content.goodsName;
