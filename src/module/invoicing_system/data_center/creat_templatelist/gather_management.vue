...
<template>
    <div id="equationList">
        <div>
            ...
            <el-table ref="multipleTable" border :header-cell-style="{'background':'#f5f7fa'}" :data="viewData"
                ...
                <el-table-column prop="formula" label="物料数量" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span>{{JSON.parse(scope.row.mid).length}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="unitName" label="集合单位" show-overflow-tooltip>
                </el-table-column>
            ...
            <div class="page-box">
                <el-pagination @current-change="pageChange" @size-change="sizeChange" background :current-page="page"
                    :page-sizes="[10, 20, 50]" :page-size="num" layout="sizes, prev, pager, next, jumper" :total="Number(allTotal)"></el-pagination>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            async init() {
                ...
                this.allTotal = data.data.length;
