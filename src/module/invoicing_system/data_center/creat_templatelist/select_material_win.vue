/*
* @Author: zhouzhe
* @Date: 2018-11-30 16:22:03
*/

<template>
	<el-dialog :title="`${title}${isSingle?'（单选）':'（多选）'}`" center :width="width" :visible.sync="openWin" @close="()=>{closeWin('cancel')}">
		<div class="dialog-content">
			<div class="showUnit" v-if="unitName">
				<span>已选择的物料单位：</span>
				<span style="color:#E1BB4A">{{unitName}}</span>
			</div>
			<div>
				<div class="sleType">
					<el-input maxlength="50" v-model="condition.name" placeholder="请输入物料名称">
						<!-- <i class="el-icon-search el-input__icon" slot="suffix" style="cursor:pointer;" @click="filterReset('filter')">
                        </i> -->
					</el-input>
				</div>
				<div class="sleType">
					<el-input maxlength="50" v-model="barCode" placeholder="请输入物料编码">
					</el-input>
				</div>
				<div class="sleType">
					<el-cascader expand-trigger="click" :options="categoryList" v-model="condition.category" placeholder="请选择分类"
					 :filterable="true" :change-on-select="true" clearable>
					</el-cascader>
				</div>
				<div class="sleType">
				...
				<div class="sleType">
					<el-button @click="filterReset('filter')" type="primary">筛选</el-button>
					<el-button type="info" @click="filterReset('reset')">重置</el-button>
				</div>

			</div>

			<div class="btn-type" v-if="!isSingle">
			...
			<div class="list-content">
				<!-- <template v-if="radio" v-for="(item,index) in nowList">
                    <el-checkbox @change="(res)=>{changeChecked(item,res)}" v-model="item['checked']" :label="item['name']"
                        border :key="index" style="margin-left:0;" class="checkbox"></el-checkbox>
                </template>
                <template v-if="!radio" v-for="(item,index) in nowList">
                    <el-radio @change="(res)=>{changeChecked(item,res)}" v-model="item['checked']" :label="item['name']"
                        border :key="index" style="margin-left:0;" class="checkbox"></el-radio>
                </template> -->
				<el-radio-group v-model="selectList" v-if="isSingle">
					<el-radio v-for="(item,i) in nowList" :class="{'setStyle':i==0}" :key="i" :label="item" style="margin-top:10px"
					 border>{{item.name}}</el-radio>
				</el-radio-group>
				<el-checkbox-group v-else v-model="selectList">
					<el-checkbox v-for="(item,i) in nowList" :class="{'setStyle':i==0}" :key="i" :label="item" style="margin-top:10px"
		...
		<template slot="footer">
			<div class="footer">
				<div class="footer-page">
					<el-pagination :pager-count="pageObj.pagerCount" :page-size="pageObj.pageSize" layout="prev, pager, next"
					 :background="true" :total="pageObj.total" :current-page.sync="pageObj.currentPage" @size-change="(res)=>{funGetPage('size-change',res)}"
					 @current-change="(res)=>{funGetPage('current-change',res)}">
					</el-pagination>
					<span class="select-num" v-if="!isSingle">
						已选:{{selectList.length}}个
					</span>
					<span class="select-num" v-else>
						已选:{{selectList.name}}
					</span>
				</div>
...
*/
	...
	let btnsName = {
		// selectAll: {
		//     id: btnType.selectAll,
		//     label: '选择全部'
		// },
		selectPage: {
			id: btnType.selectPage,
			label: '选择本页'
		},
		// cancelAll: {
		//     id: btnType.cancelAll,
		//     label: '取消全部'
		// },
		cancelPage: {
			id: btnType.cancelPage,
			label: '取消本页'
		}
	};
	...
	export default {
		...
		methods: {
			...
			getMaterialList(subObj) {
				...
				arr = this.filterListByAttr(arr, this.barCode, 'barCode');
				arr = this.filterListByAttr(arr, this.typeValue + '', 'type');
		...
		watch: {
			...
			selectBtn: function () {
				...
				switch (this.selectBtn) {
					case btnType.selectAll:
						this.btns.push(btnsName.cancelPage);
						break;
					...
					case btnType.none:
					case btnType.cancelAll:
						this.btns.push(btnsName.selectPage);
						break;
