<template>
	<win :align="'center'" @winEvent="closeSelfWin" :width="1024" :height="600" :ok="btnOk">
		...
		<div slot="content" id="select-batch">
			...
			<div class="batch-title">
				<ul>
					<li>物料名称:{{needItem.itemName}}</li>
					<li>物料单位:{{unitStr}}</li>
					<li>默认单位:{{defaultUnit}}</li>
					<li>申请单位:{{needItem.selectName}}</li>
				</ul>
				<ul>
					<li>保质期:{{validity}}</li>
					<li>分类:{{cateArr}}</li>
					<li>最小单位:{{minUnit}}</li>
					<li>需求数量:{{getSurplus(needItem.num)}}</li>
				</ul>
			</div>
...
<script>
	...
	export default {
		...
		props: ['pObj', 'needItem', 'wid', 'shopId'],
		methods: {
			async init() {
				//获取批次列表
				let res = await http.All([{
					...
					data: {
						mid: this.needItem.id,
						shopId: this.shopId
					}
				},
				{
					httpId: 'getMaterialBatch',
					data: {
						mid: this.needItem.id,
						wid: this.wid || '',
						shopId: this.shopId
				...
				let unitArr = [];
				if (this.detail.unit && this.detail.unit.length > 0) {
					for (let item of this.detail.unit) {
						unitArr.push(item.name);
						if (item.isDefault == 1) {
							this.defaultUnit = item.name;
						}
						if (item.isMin == 1) {
							this.minUnit = item.name;
						}
					}
				}
				let cateArr = [];
				...
				this.validity = `${this.detail.validity}${this.validityType[this.detail.validityType]}`;
				this.cateArr = cateArr.join(',');
				this.unitStr = unitArr.join(',');
				this.allList = res[1].data.list;
				this.changeList = utils.deepCopy(this.allList);
			...
			getSurplus(data) {
				return global.comUnit(data, this.needItem.selectValue, this.needItem.selectName, this.minUnit);
				// return parseInt((data/this.needItem.selectValue)*1000)/1000
			},
			search() {
				let arr = [];
			...
			getmin(e, item) {
				let res = /^\d*\.?\d+$/;
				let value = Number(this.needItem.selectValue);
				item.outNum = res.test(item.outNum) ? item.outNum : 0;
				if (item.outNum != '') {
					if (Number(item.outNum)*value > Number(item.surplus)) item.outNum = parseInt(Number(item.surplus)*1000/value)/1000;
					e.target.value = (item.outNum + '').replace(/^(\d*)\.?\d{4}$/g, parseInt(item.outNum * 1000) / 1000);
