<template>
	<section>
		<div id="memberDetail">
			...
			<div style="width:100%;height:45px;margin:20px 0px;">
			...
			<com-table v-if="bannerIndex == 0" :listHeight='80' :listName="'历史记录'" :key="index" :allTotal="count" :showTitle='1' :introData="listInfo.list" :titleData="titleList">
				<div slot="con-0" slot-scope="props">{{translateTime(props.data.createTime,'1')}}</div>
				<div slot="con-1" slot-scope="props">{{judgeType(props.data)}}</div>
				<div slot="con-2" slot-scope="props">{{filterType(props.data.type)}}</div>
				<div slot="con-3" slot-scope="props">
					<template v-if="props.data.type == 1 || props.data.type == 2 || props.data.type == 6 || props.data.type == 7 || props.data.type == 12 || props.data.type == 13">
						{{props.data.amount}}
					</template>
					<template v-else>
						{{props.data.point}}
			...
			<com-table v-if="bannerIndex == 1" :listHeight='80' :listName="'更改记录'" :key="index" :showTitle='1' :introData="listInfo.list" :titleData="titleList_q" :allTotal="count">
				<div slot="con-0" slot-scope="props">{{translateTime(props.data.createTime,'1')}}</div>
				<div slot="con-1" slot-scope="props">{{filterType(props.data.type)}}</div>
				<div slot="con-2" slot-scope="props">{{judgeType(props.data)}}</div>
				<div slot="con-3" slot-scope="props">
				...
				<div slot="con-4" slot-scope="props">{{props.data.userName}}</div>
			</com-table>
			<!-- 积分余额变动图表 -->
			<com-table v-if="bannerIndex == 2 || bannerIndex == 3" :listHeight='80' :listName="listName" :key="index" :showTitle='1' :introData="listInfo.list" :titleData="titleList_j" :allTotal="count">
				<div slot="con-0" slot-scope="props">{{translateTime(props.data.createTime,'1')}}</div>
				<div slot="con-1" slot-scope="props">{{judgeType(props.data)}}</div>
