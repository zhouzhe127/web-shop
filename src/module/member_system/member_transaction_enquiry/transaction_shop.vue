			...
			<div @click.stop class="detDivBox elshop">
				...
				<div class="detH3" style="text-align: left;max-height: 500px;overflow: auto;">
					<el-tabs v-model="activeName">
						...
						<el-tab-pane label="全部" name="first">
							<el-checkbox v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.shopName" border size="medium">
							</el-checkbox>
						</el-tab-pane>
						<el-tab-pane label="微信" name="second">
							<el-checkbox v-if="item.ischain == 3" v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.shopName" border size="medium"></el-checkbox>
						</el-tab-pane>
						<el-tab-pane label="门店" name="third">
							<el-checkbox v-if="item.ischain != 3" v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.shopName" border size="medium"></el-checkbox>
						</el-tab-pane>
			...
			selCommon(type) {
				for (let i = 0; i < this.showShopList.length; i++) {
					if (this.activeName == 'first') {
						...
						// } else {
						// 	if (this.showShopList[i].storeAreaId == this.areaList.flag) {
						if (this.showShopList[i].ischain == 3) {
							this.showShopList[i].selected = type;
						}
						// 	}
						// }
