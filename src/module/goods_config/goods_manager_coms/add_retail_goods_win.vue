<template>
	<div id="detail-addgoods">
			...
			<section>
				...
				<section style="width:100%;overflow:hidden;">
					<el-form :model="good" ref="good" label-width="80px">
						...
						<el-form-item required label="分类">
							<span @click="openCommonWin('category')" class="addIcon">
								<i class="el-icon-circle-plus-outline"></i>
								<span>添加分类</span>
								<span class="sign" v-for="cat in selectCategory" :key="cat.id">{{cat.name}}</span>
							</span>
						</el-form-item>
							...
							<section style="width:100%;margin-top:48px;">
								...
								<div class="good-image" v-for="(good,index) in goodPicList" :key="index" style="margin:10px;">
									...
									<h3 style="text-align:center;">
										<i @click="delImage(index)" class="el-icon-remove-outline"></i>
										<span @click="delImage(index)">移除</span>
									</h3>
								</div>
							</section>
						</el-form-item>
					...
					<el-form :model="good" ref="good" label-width="85px">
							...
							<div class="goodsSizeBox">
								<section v-for="(item,index) in titleList" :key="index">
									<section class="sizeTop">
										<el-autocomplete v-if="item.isadd" class="inline-input" v-model="item.name" size="small" @focus="getFocus(item,index)" @blur="addSpecValue(item,true)" :fetch-suggestions="querySearch" placeholder="请输入内容" :name="'name'" :label="'name'" @select="handleSelect"></el-autocomplete>
									...
									<section class="sizeBox">
										<section class="sizeI" v-for="(size,i) in item.value" :key="i">
											<el-autocomplete class="inline-input" v-if="size.isadd" v-model="size.name" size="small" @focus="getFocus(item,index,i)" @blur="addSpecValue(size,false,item.id)" :fetch-suggestions="querySearch" @select="handleSelect" style="width:100px;"></el-autocomplete>
						...
						<el-form-item label="耐克直营">
							...
							<section style="margin-top:20px;">
								<el-table ref="multipleTable" size="small" :header-cell-style="{'background-color':'#f5f7fa'}" :data="groupData" border style="width: 100%">
									...
									<el-table-column min-width="120" align="center" prop="price" label="图片">
										<template slot-scope="scope">
											<span class="titleNames" v-if="titleList.length==1">
												<span v-if="scope.row.image == ''" class="addIcon">
													<i class="el-icon-circle-plus-outline"></i>
													<span>添加图片</span>
												</span>
												<section v-else class="good-image-div" id="image" style="height:40px;margin:5px 0;">
													<img v-if="!!scope.row.image" :src="imgHost+scope.row.image" style="height:100%;">
													<img v-else src="../../../res/images/busis.png" alt="商品" style="height:100%;" />
												</section>
												<form enctype="multipart/form-data" :id="'img_upload'+scope.row.specId+'a'" style="position:absolute;top:0;">
													<input type="file" @change="changeGoodsImg({item:scope.row,formId:'img_upload'+scope.row.specId+'a'})" accept="image/jpeg,image/png,image/gif,image/tiff" name="image" class="good-image-file" style="width:120px;height:40px;position:absolute;top:5px;" />
												</form>
											</span>
											<span v-if="titleList.length==2" class="titleName" v-for="(item,i) in scope.row.value" :key="i" style="position:relative">
												<span v-if="item.image == ''" class="addIcon">
													<i class="el-icon-circle-plus-outline"></i>
													<span>添加图片</span>
												</span>
												<section v-else class="good-image-div" id="image" style="height:40px;margin:5px 0;">
													<img v-if="!!item.image" :src="(item.image.indexOf('http')>-1)?item.image: imgHost+item.image" style="height:100%;">
													<img v-else src="../../../res/images/busis.png" alt="商品" style="height:100%;" />
												</section>
												<form enctype="multipart/form-data" :id="'img_upload'+scope.row.id+'a'+item.id" style="position:absolute;top:0;">
													<input type="file" @change="changeGoodsImg({item:item,formId:'img_upload'+scope.row.id+'a'+item.id})" accept="image/jpeg,image/png,image/gif,image/tiff" name="image" class="good-image-file" style="width:120px;height:40px;position:absolute;top:5px;" />
												</form>

											</span>
											<span v-if="titleList.length==3" class="titleName" v-for="(item,i) in scope.row.value" :key="i">
												<span class="titleNames" v-for="(items,i) in item.value" :key="i" style="position:relative">
													<span v-if="items.image == ''" class="addIcon">
														<i class="el-icon-circle-plus-outline"></i>
														<span>添加图片</span>
													</span>
													<section v-else class="good-image-div" id="image" style="height:40px;margin:5px 0;">
														<img v-if="!!items.image" :src="(items.image.indexOf('http')>-1)?items.image: imgHost+items.image" style="height:100%;">
														<img v-else src="../../../res/images/busis.png" alt="商品" style="height:100%;" />
													</section>
													<form enctype="multipart/form-data" :id="'img_upload'+scope.row.id+'a'+item.id+items.id" style="position:absolute;top:0;">
														<input type="file" @change="changeGoodsImg({item:items,formId:'img_upload'+scope.row.id+'a'+item.id+items.id})" accept="image/jpeg,image/png,image/gif,image/tiff" name="image" class="good-image-file" style="width:120px;height:40px;position:absolute;top:5px;" />
													</form>
												</span>
											</span>
										</template>
						...
						<el-form-item required label="仓库选择">
							<el-button size="mini" @click="openCommonWin('waseHouse')" style="width:82px;" type="primary">选择仓库</el-button>
							<span v-if="waseHouseitem.name&&waseHouseitem.areaObj.areaName">已选择{{waseHouseitem.name+"/" +waseHouseitem.areaObj.areaName}}</span>
						</el-form-item>
					</el-form>
				</section>
			</section>
		...
		<component :is="showCom" :pObj="comObj" :pGoodsList="comObj" @throwWin="closeWaseWin" @throwCommonWin="closeCommonWin"></component>
	</div>
</template>
<script>
/*
...
export default {
		...
		/*
			{
				...
				brandList: this.brandList,//品牌列表
				suppierList: this.suppierList, //供应商列表
				specList:this.specList,//零售商品列表
			}
		*/
	},
	mounted() {
		...
		this.specValueList = this.specList;
	...
	methods: {
		...
		delImage(index) {
			this.$store.commit('setWin', {
				...
				callback: callbackRes => {
					if (callbackRes == 'ok') {
						this.goodPicList.splice(index, 1);
					}
				}
			});
		},
		...
		async getFocus(item, index, i) {
			...
			if (i >= 0) {
				let list = await http.getListBySpecId({
					data: {
						specId: item.id
					}
				});
				// let list = this.getspecVal(item.id);
				for (let i = 0; i < list.length; i++) {
					list[i].value = list[i].name;
				}
				this.specValueList = list;
				// this.specValueList = JSON.parse(item.specValue);
			} else {
				this.specValueList = utils.deepCopy(this.specList);
			}
		},
		//添加规格接口--失去焦点
		...
		async createGood() {
			...
			if (isOk) {
				...
				if (this.editGoodsId) {
					detial.id = this.editGoodsId;
					let gooddetial = await http.goodRetailEdit({
						data: detial
					});
					if (gooddetial) {
						this.alertWin('修改商品成功!');
					}
			...
			for (let i = 0; i < arr.length; i++) {
				if (titleList.length > 1) {
					...
					for (let key of keyss) {
						...
						newObj.specValueIds = arr[i].specId;
					}
					goodArr.push(newObj);
				}
			}
		...
		async openCommonWin(flag) {
			...
			switch (flag) {
				...
				case 'waseHouse':
					this.showCom = 'waseHouse';
					console.log(this.suppierId);
					this.comObj = {
						selectId: this.waseHouseitem.id
							? this.waseHouseitem.id
							: '',
						title: '选择仓库',
						isArea: true,
						areaId:
							this.waseHouseitem.areaObj &&
							this.waseHouseitem.areaObj.id
								? this.waseHouseitem.areaObj.id
								: ''
						// only:2,
						// shopId:this.shopId
		...
		closeWaseWin(flag, data) {
			this.showCom = '';
			if (flag != 'ok') return;
			this.waseHouseitem = data;
		},
		//请求数据
		...
		async asyncRequest() {
			...
			if (res) {
				...
				//通过详细，获取分类信息
				let catArr = [];
				let category = utils.deepCopy(this.category).splice(1, 1);
				for (let i = 0; i < category.length; i++) {
					if (res.categoryId == category[i].id) {
						catArr.push(category[i]);
						break;
					}
					for (let j = 0; j < category[i].child.length; j++) {
						if (res.categoryId == category[i].child[j].id) {
							catArr.push(category[i].child[j]);
							break;
						}
					}
				}
				this.selectCategory = catArr;
				//品牌信息
				for (let i = 0; i < this.brandList.length; i++) {
					if (res.brandId == this.brandList[i].id) {
						this.selectBrandObj = this.brandList[i];
					}
				}
				this.goodPicList = JSON.parse(this.good.image); //商品图片
				...
				if (titleList.length > 1) {
						...
						for (let j = 0; j < goodArr[i].value.length; j++) {
							...
							if (titleList.length == 3) {
								...
								for (
									...
									if (
										goodArr[i].value[j].value[k].price ==
											undefined &&
										goodArr[i].value[j].value[k].barCode ==
											undefined
									) {
										goodArr[i].value[j].value.splice(k, 1);
										k--;
									}
								}
							} else {
								...
								if (
									goodArr[i].value[j].price == undefined &&
									goodArr[i].value[j].barCode == undefined
								) {
									goodArr[i].value.splice(j, 1);
									j--;
								}
							}
						}
					...
					for (let m = 0; m < groupData.length; m++) {
						let spuId = groupData[m].specValueIds.split(',');
						groupData[m].specId = groupData[m].specValueIds;
						for (let i = 0; i < goodArr.length; i++) {
							titleList[0].value[i].isadd = false;
							if (groupData[m].specValueIds == goodArr[i].id) {
								groupData[m].name = goodArr[i].name;
								groupData[m].isadd = false;
							}
						}
					}
