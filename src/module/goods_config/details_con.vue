	...
	<section id="details_con" v-cloak>
	  ...
	  <section style="width:100%;height:50px;">
		<div v-if="ischain == 1|| ischain == 2" style="float:left;margin-right:10px;">
		  <el-select
			...
			style="width:150px;"
		  >
			<el-option v-for="item in goodSec" :key="item.id" :label="item.name" :value="item.id"></el-option>
		  </el-select>
		</div>
		<!-- 分类选择 -->
		<elCategory
	  ...
	  <div class="animate-div" v-if="!load">
		<div class="img-div">
		  ...
		  <div style="font-size:20px;color:#999;">商品加载中,请稍后...</div>
		</div>
	  </div>
	  <!--	商品展示  -->
	  <div v-if="selectTab==0" class="commodity-box">
		<div class="media-div" ref="addGoods">
		  <section
			class="add_goods_list list-div"
			:style="{'height':listHeight+'px'}"
			v-on:click="openAddWin({})"
			v-show="load"
		  >
		...
		<div class="media-div" v-for="(item,index) in nowGoods" :key="index">
		  <section
			class="list-div"
			:style="{'height':listHeight+'px'}"
			v-on:click="openAddWin(item)"
		  >
			<img
			  v-if="!!item.imageName"
			  class="imageClass"
			  :src="(item.imageName.indexOf('http')>-1)?item.imageName: imgHost+item.imageName"
			  :alt="item.goodsName"
			>
			...
			<section
			  class="shopTag"
			  style="width: 70px;left: 10px;"
			  v-if="ischain == 1 || ischain == 2"
			>
			...
			<section class="comprice">
			  ...
			  <section
				...
				class="fl"
				style="height: 55px;line-height: 25px;width: 100%;"
			  >
				<section>
				  <section
					v-if="item.isGroup==1 && item.isStock == 1"
				  ...
				  <section v-else style="width: 100%;text-align: left;" class="fl">
					<span class="fl" style="width: 42px;">库存：</span>
					<span class="fl stockStyle">{{item.goodsNum ? item.goodsNum :'未开启库存'}}</span>
				  </section>
				</section>

				<section>
				  <section
					v-if="item.isGroup==1 || (item.isGroup==1 && item.isSeasonal==1)"
					class="fr price-align"
					style="float:left;"
				  >
		...
		>
		  ...
		  >
			<template slot-scope="scope">
			  <span
				v-if="ischain=='1'||ischain=='2'"
				@click="openAddWin(scope.row)"
				:style="{color:scope.row.id<10000?'#fe9200':'#2ea7e0',cursor:'pointer'}"
			  >{{scope.row.goodsName}}</span>
			  <span
				v-if="ischain=='0'||ischain=='3'"
				@click="openAddWin(scope.row)"
				style="color:#2ea7e0;cursor:pointer"
			  >{{scope.row.goodsName}}</span>
			</template>
		  </el-table-column>
		  <el-table-column
		  ...
		  <el-table-column fixed="right" width="150" align="center" prop="totalDay" label="操作">
			<template slot-scope="scope">
			  ...
			  <span
				...
				style="color:rgb(108, 194, 230);cursor:pointer"
				@click="updownWin(scope.row)"
			  >上架</span>
