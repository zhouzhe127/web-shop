...
//        },
		...
		difference() {
			...
			for (let i = 0; i < this.allList.length; i++) {
				if (this.allList[i].id == id) {
                    ...
                    this.id = v.id;
                    this.statsName=v.status == -1 ? '未开通' : v.status == 0 ? '关闭' : '开启';
                    if(v.status==-1){
                        this.currentList = [
                            {
                                name: '全时段',
                        ...
                        this.currentList = [
                            {
                                ...
                                time: '--',
                                cityGoods:
                                (v.goodsIds == '' ? 0 : v.goodsIds.split(',').length) +
                                (v.packageIds == '' ? 0 : v.packageIds.split(',').length),
                                peopleGoods: v.perGoodsIds.length,
                                mustGoods:
                                (v.mustGids == '' ? 0 : v.mustGids.split(',').length) +
                                (v.mustPids == '' ? 0 : v.mustPids.split(',').length),
                                mustSonGoods:
                                (v.mustSonGids == '' ? 0 : v.mustSonGids.split(',').length) +
                                (v.mustSonPids == '' ? 0 : v.mustSonPids.split(',').length),
                                mustNum: v.mustNum
                            }
                        ];
					}
			...
			for (let i = 0; i < this.allList.length; i++) {
				if (this.allList[i].id == id) {
					...
					this.goodCom = {
						...
						packageIds: v.packageIds == '' ? [] : v.packageIds.split(','), //市别套餐
						mustGids: v.mustGids == '' ? [] : v.mustGids.split(','), //必点商品id集合
						mustPids: v.mustPids == '' ? [] : v.mustPids.split(','), //必点套餐id集合
						mustNum: v.mustNum, //必点数量
						mustSonGids: v.mustSonGids == '' ? [] : v.mustSonGids.split(','), //必点商品子菜id集合
						mustSonPids: v.mustSonPids == '' ? [] : v.mustSonPids.split(','), //必点商品子套餐id集合
        ...
        addXia(){
            ...
            for(let i=0;i<this.goodCom.mustGids.length;i++){//添加必点选择过的下架商品
            ...
            for(let i=0;i<this.goodCom.mustPids.length;i++){//添加必点选择过的下架套餐
                for(let j=0;j<this.allPackX.length;j++){
                    if((this.goodCom.mustPids[i]==this.allPackX[j].id)){
                        if(!this.allPackX[j].packageName.includes('(已下架)')){
		...
		async getpackagelist() {
			...
			let res = false;
			this.version = await this.ShopGetExtra();
			if (version && this.version.otherConfigVer == version.otherConfigVer) {
				res = true;
			} else {
				res = false;
				storage.session('httpGoodVersion', this.version);
