...
export default {
  ...
  methods: {
	...
	async handleMoveTemp(item, index, dir) {
	  ...
	  let afterSort = null;
	  console.log(dir, item.sort, "0000");
	...
	async editSucess(item, index, sindex, column, type) {
	  ...
	  let isVip = price.isVip == true ? "1" : "0";
	  let reg = /[0-9]*/;
	  console.log(reg.test(this.tempEditPrice), "222");
	  // if(!reg.test(Number(this.tempEditVipPrice)) || !reg.test(Number(this.tempEditPrice))){
	  // 		this.$alert('请输入正确的数字', '输入错误', {
	...
	async getClearPricetemplate() {
	  let data = await http.clearPricetemplate({});
	  console.log(data);
	},
	// 删除模板
