...
-->
	...
	<div id="attr_manager">
		...
		<component :is="showCom" @throwAddAttrWin='closeGetAddAttrWin' @throwSyncAttr="funGetSyncAttr" :pObj="comObj"></component>
	</div>
</template>
<script>
/*
...
export default {
	...
	created() {
		...
		this.isBrand =
			this.userData.currentShop.ischain == '1' ||
			this.userData.currentShop.ischain == '2'
				? true
				: false;
		let arr = [
			{
	...
	methods: {
		...
		paging() {
			...
			this.showTaste = this.taste.slice(
	...
	components: {
		attrAdd: () =>
			import(/*webpackChunkName:'attr_add_win'*/ './attr_coms/attr_add_win'),
		attrAsync: () =>
			import(/*webpackChunkName:'attr_sync_win'*/ './attr_coms/attr_sync_win')
	},
	filters: {
		divide(str) {
			if (str.length < 7) {
