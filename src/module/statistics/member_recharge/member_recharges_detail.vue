		...
		<section v-if="isShow == 'all'">
			...
			<section style="margin-bottom:20px;">
				<!-- <com-table :listHeight='80' :listWidth="1400" :showHand="false" :titleData="titleList" :introData="staticLists">
					...
					<div slot="con-11" slot-scope="props" @click="openPayment(scope.row,'one')">
						{{scope.row.totalOtherPay}}
					</div>
				</com-table> -->
			...
			<!-- <section>
				<com-table :listHeight='80' :listWidth="1400" :showHand="false" :titleData="shoptitleList" :introData="formList" :listName="'会员充值统计'" :key="index" :showTitle='1' :allTotal="count">
					<div slot="con-0" slot-scope="props" @click="openDetail(scope.row)">
						查看账户
					</div>
					...
					<div slot="con-8" slot-scope="props">
						{{getpaytypeName(scope.row.payType) || payTypeList[scope.row.payType].paymentName}}
					</div>
					<div slot="con-12" slot-scope="props" @click="openDiscount(scope.row)">
						{{scope.row.giftCoupon}}
					</div>
				</com-table>
			</section> -->
			<!-- 下面的表格 -->
			...
			async Export() {
				await http.exportRechargeData({
					data: {
						...
						showDay: this.oneData, //查看一家店铺传递的日期
						page: this.page, //页码
