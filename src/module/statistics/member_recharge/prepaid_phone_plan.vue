						...
						<!-- <template v-if="!isSingle"> -->
							<el-tab-pane label="全部" name="first">
								<el-checkbox v-if="(item.storeAreaId == areaList.flag) || areaList.flag == -1" v-for="(item,index) in showShopList"
											 :key="index" v-model="item.selected" :label="item.name" border size="medium">
								</el-checkbox>
							</el-tab-pane>
							<el-tab-pane label="微信" name="second">
								<el-checkbox v-if="item.type == 1 && (item.storeAreaId == areaList.flag || areaList.flag == -1)" v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.name" border size="medium"></el-checkbox>
							</el-tab-pane>
							<el-tab-pane label="POS收银" name="third">
								<el-checkbox v-if="item.type == 2 && (item.storeAreaId == areaList.flag || areaList.flag == -1)" v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.name" border size="medium"></el-checkbox>
							</el-tab-pane>
						<!-- </template> -->
						<!-- <template v-if="isSingle">
							<el-tab-pane label="全部" name="first">
...
export default {
	...
	* */
	...
	methods: {
		async getDepositPlanList(){
			//console.log('2222');
			let data = await http.getDepositPlanList({
				data:{
		...
		init() {
			...
			if(this.isSingle){//单选
				...
				if(this.singleId.length>0){
					for(let i=0;i<this.allShop.length;i++){
						if(this.singleId.includes(this.allShop[i].id)){
							this.singleName=this.allShop[i].name;
							break;
						}
					}
				}else {
				...
				this.title=this.shopIds.length>0?`已选择${this.shopIds.length}充值方案`:'请选择充值方案';
			}
		...
		searchShop(){
			...
			this.showShopList = this.shopList.filter(e=>{
				if(e.shopName.indexOf(this.name) != -1){
					return e;
				}
			});
		},
		...
		selCommon(type){
			...
			for(let i = 0; i < this.showShopList.length; i++){
				if(this.activeName == 'first'){
					...
					if(this.areaList.flag == '-1' && this.showShopList[i].type == 1){
						this.showShopList[i].selected = type;
					}else{
						if(this.showShopList[i].storeAreaId == this.areaList.flag){
							this.showShopList[i].selected = type;
					...
					if(this.areaList.flag == '-1' && this.showShopList[i].type == 2){
						this.showShopList[i].selected = type;
					}else{
						if(this.showShopList[i].storeAreaId == this.areaList.flag){
							this.showShopList[i].selected = type;
