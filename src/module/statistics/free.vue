...
<template>
	<div class="delete">
		<template v-if="!detailShow">
			...
			<div class="main">
				<!--店铺列表-->
				<freeStore :store="orderStoreList" :order="goodsData" :otherData="orderStoreSend" :step="showStep" :loading="loading" @emit="getStep"></freeStore>
			</div>
		</template>
		<!--退品详情-->
...
export default {
	...
	methods: {
		...
		async getGoods() {
			...
			let data = await http.moreFreeByName({
				data: {
		...
		async exportGoods() {
			...
			if (!this.goodsData) {
				...
				await http.exportMoreFreeByName({
					data: {
						trueShopId: this.shopId,
						startTime: parseInt(this.timeObj.startTimeStore / 1000),
						endTime: parseInt(endTimeStore / 1000),
