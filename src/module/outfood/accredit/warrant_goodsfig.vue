	...
	<div class="shopList" id="releGoods">
		<div class="shopList">
			...
			<ul class="shopTitle title">
				...
				<li style="width: 42%;">外卖商品</li>
				<li style="width: 42%;">闪店系统(已关联{{relNum}}/{{allNum}})</li>
			</ul>
		...
		<addfood-win ref="addwin" v-if="addWin.winshow" :item="item" :type="addWin.type" :goodsList="goodsList" :width="addWin.width"
		 :height="addWin.height" :title="addWin.title" @winEvent="addfoodwin"></addfood-win>
		<relationWin ref="relationwin" v-if="relationWin.winshow" :width="relationWin.width" :item="relationItem"
		 :changeConfig="relationWin.changeConfig" :seloneAir="relationWin.seloneAir" :seltwoAir="relationWin.seltwoAir"
		 :height="relationWin.height" :title="relationWin.title" @relationEvent="relationevn"></relationWin>
	</div>
</template>
<script>
		...
		methods: {
			...
			success: function () {
				if (this.relNum < this.allNum) {
					...
					this.$confirm('您还有商品未关联，确定要关闭页面吗？', '提示', {
...
<style lang="less" scoped>
	...
	#warrant .shopList .content .sdgoods ul {
		display: flex;
	}
