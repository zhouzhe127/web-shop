		...
		<div id="tan" slot="content" slot-scope="props" style="height:100%">
			<section id="getGoods" style="padding: 20px;">
				...
				<section class="comList" v-if="selectgoods.length>0">
					<mul-select @selOn="listChange" :styles="{backgroundColor: '#F1F1F1'}" :list="selectgoods" :name="'goodsName'" :isradio="true" :selects="goodsIndex" :keys="'id'"></mul-select>
				</section>
				<section class="place fl">
				...
				<section class="comList">
					<mul-select @selOn="packChange" :styles="{backgroundColor: '#F1F1F1'}" :list="fixed" :name="'packageName'" :isradio="true" :selects="fixedIndex" :keys="'id'"></mul-select>
				</section>
			</section>
		</div>
	</win>
...
export default {
	...
	mounted() {
		this.init();
		this.goodsCom = storage.session('goodList');
		this.goodReady();
