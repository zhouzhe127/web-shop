...
<template>
	<div id="pay-window">
		...
		<div class="pay-window-box">
			...
			<div class="rightHalf">
				<el-input class="fl" v-model="name" placeholder="请输入自定义名称" maxlength="4" style="width:179px;"></el-input>
		...
		<div class="pay-window-box" v-show="opentheFunction">
			...
			<el-switch v-model="payStatus" active-color="#E1BB4A" inactive-color="#dcdfe6">
			</el-switch>
		</div>
		<!-- 营业时间 -->
		...
		<div class="pay-window-box clearfix" v-if="type == '1'">
			...
			<div class="rightHalf">
				<el-input class="fl" placeholder="请输入正整数" v-model="sendingfee" maxlength="6" onkeyup="value=value.replace(/[^\d\.]/g,'')" @blur="keepValue('sendingfee')" style="width:179px;">
		...
		<div class="pay-window-box clearfix" v-if="type == '1'">
			...
			<div class="rightHalf">
				<el-input class="fl" placeholder="请输入正整数" v-model="estimatedtime" maxlength="6" onkeyup="value=value.replace(/[^\d.]/g,'')" style="width:179px;">
		...
		<div class="pay-window-box clearfix" v-if="type == '0'">
			...
			<div class="rightHalf">
				...
				<el-tooltip class="item" effect="dark" content="我们会根据您设置的配餐时间自动呼叫第三方配送,以保证配送的及时性,配餐时间不能超过60分钟哦~" placement="right">
		...
		<div class="pay-window-box" v-if="type == '1'">
			...
			<el-switch v-model="mealStatus" active-color="#E1BB4A" inactive-color="#dcdfe6">
			</el-switch>
		</div>
		<!-- 备餐时间 -->
		...
		<div class="pay-window-box clearfix">
			...
			<span class="fl" v-if="type == '0'">
					<mulSelect :list.sync="configurationList" :selects.sync="configurationSelect" :name='"name"' :key='"id"' :styles="{'border-radius':'4px','margin-right':'8px'}"></mulSelect>
				</span>
		...
		<div class="pay-window-box clearfix" v-if="type == '0' || type == '2'">
			...
			<el-switch class="fl" v-model="personStatus" active-color="#E1BB4A" inactive-color="#dcdfe6" style="margin-top: 10px;">
			</el-switch>
			<div class="fl handle-tips">
				<i></i> 关闭后,默认用餐人数为1人
			</div>
		...
		<div class="pay-window-box clearfix" v-if="(type == '0' || type == '2') && personStatus">
			...
			<div class="rightHalf">
				...
				<el-input class="fl" placeholder="请输入正整数" maxlength="6" v-model="mealsNum" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:179px;">
		...
		<div class="pay-window-box clearfix">
			...
			<div class="rightHalf">
				...
				<el-switch v-model="manypeopleStatus" active-color="#E1BB4A" inactive-color="#dcdfe6">
				</el-switch>
		...
		<div class="pay-window-box clearfix" v-if="entertainingStatus">
			...
			<div class="rightHalf">
				...
				<div class="handle-tips" style="margin-left: 170px;">
					<i></i> 限100字
				</div>
			</div>
		</div>
...
export default {
	...
	data() {
		return {
			...
			fixed: '',
			ladder: '',
			entertainingStatus: false, //跑马灯设置
			entertainingTxt: '' //跑马灯内容
		};
	},
	watch: {
		// 'distances': {
	...
	methods: {
		...
		getItemInfo: function() {
			...
			//跑马灯
			if (item.marquee != '') {
				this.entertainingStatus = true;
				this.entertainingTxt = item.marquee;
			}
			//当外卖的时候 给使用时间段赋值
			//0 与门店营业时间保持一致
			//1 单独设置外卖时间
		...
		setItem: function() {
			...
			//跑马灯
			if (this.entertainingStatus) {
				item.marquee = this.entertainingTxt;
			} else {
				item.marquee = '';
			}
		},
