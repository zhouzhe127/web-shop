	...
	<div id="self-help" class="self-help">
		<section v-if="!show">
			...
			<div class="self-help-box clearfix">
				...
				<span class="fl">
					<mulSelect :list.sync="goodsType" :selects.sync="goodsSelect" :styles="{'backgroundColor':'rgb(240,240,240)','marginRight':'8px'}" :name='"name"'
					    :key='"id"'></mulSelect>
				</span>
			</div>
			<!-- 结账功能 -->
			...
			<div class="self-help-box clearfix">
				...
				<!-- <span :class="selfStatus == false ?'off check-span' : 'on check-span'" v-on:click="iselfStatus"></span> -->
		...
		<pay v-else  @winEvent='winEvent' :opentheFunction='opentheFunction' :upPayInfo='upPayInfo' :isUpdata='isUpdata'  :modifyindex='modifyindex' :checkoutMenu='checkoutMenu'></pay>
	</div>
</template>
<script>
...
.add_btn {
	background: url(../../res/images/add.png) no-repeat 20px center #f8941f;
	height: 40px;
	line-height: 40px;
	text-align: center;
	width: 200px;
...
.on-span {
	...
	background: #f8941f url("../../res/images/add.png") center no-repeat;
}
