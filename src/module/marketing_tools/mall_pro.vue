	...
	<section>
		<div v-if="!isEdit" class="mall-box" id="pro">
			<div v-if="ischain != '1' && ischain != '2'" class="addFun">
				<a href="javascript:void(0);" class="firstBtn fr" style="position:absolute;top: 12px;right: 60px;line-height: 40px;display: block;background-color: #f8931f;color: #fff;width: 150px;"
				    @click="openEdit">添加推广素材</a>
			</div>
			<div class="fl" style="width: 75%;margin-right: 5%;">
				<com-table :listHeight='80' :showHand="false" :listName="'推广素材列表'" :key="index" :showTitle='2' :listWidth="1300" :introData="proList"
					...
					<!-- <div v-else class="operate" slot="con-3"></div>
                    <h2 class="tips" v-if="props.data.length == 0" slot="con-3">您未添加任何推广素材,快去添加吧!</h2>  -->
				</com-table>
			</div>
