<template>
	<section class='birth_act'>
		...
		<!-- 距离最后一次消费 -->
		<div class="online-box clearfix">
			<span class="online-sub fl required" style="width:170px;">距离最后一次消费天数</span>
			<div class="rightHalf">
				<el-input class='fl' placeholder="请输入天数" v-model="number" maxlength="3" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:179px;">
					<template slot="suffix">天</template>
				</el-input>
				<div class="fl handle-tips">
					<i></i> 一次活动，任意会员达到未消费天数或倍数时，送券
				</div>
			</div>
		</div>
		<!-- 返券 -->
		<div class="online-box clearfix">
			...
			<div class="rightHalf">
				<el-button class="fl" type="primary" icon="el-icon-plus" @click="addCoupon" style="width:179px;">关联优惠券</el-button>
				<span class="coupon_name" v-if="selectCoupon.length > 0">(已关联{{selectCoupon.length}}张)</span>
			</div>
		</div>
		<!-- 活动通知 -->
		<div class="set-line">
...
export default {
	...
	methods: {
		...
		checkForm: function() {
			...
			if (Number(this.number) < 1 || Number(this.number) > 365) {
				this.valiData('请填写消费天数1-365');
				return false;
			}
			return true;
		},
		...
		async getActivityDetail(item) {
			...
			this.number = couponIds.notConsumeDays; //距离消费天数
			this.selectCoupon = couponIds.couponIds; //关联的优惠券
