...
<template>
	<div id="screening">
		...
		<div class="online-box clearfix">
			...
			<a href="javascript:void(0);" class="blue fl" style="width: 200px;margin-right:10px;" @click="opengResult">筛选</a>
			<a href="javascript:void(0);" class="blue fl" style="width: 300px;" @click="opengResult('1')">保存条件,根据活动时间筛选用户</a>
		</div>
		<!-- 结果弹窗 -->
		<resultWin v-if="showWin" @getAppliedWin='getResult' :chooseData='chooseData'></resultWin>
	</div>
</template>
<script>
...
export default {
	...
	methods: {
		...
		getResult: function(res, data) {
			//选择业务类型
			if (res == 'ok') {
				this.memberNum = data;
				this.returnStore(); //返回页面
			}
			this.showWin = false;
		},
		opengResult: function(type) {
			if (!this.checkForm(type)) return;
			// if (utils.isEmptyObject(this.chooseData)) {
			// 	this.valiData('请选择条件进行筛选');
			// 	return false;
			// }
			this.showWin = true;
		},
		...
		checkForm: function(type) {
			...
			for (let item of this.memberInfo) {
				...
				if (item.id == 4 && item.status) {
					...
					this.chooseData.levelId = this.levelSelect.join(',');
				}
				//注册日期
	...
	components: {
		...
		resultWin: () =>
			import ( /*webpackChunkName: 'activity_result'*/ './activity_result'),
	},
	watch: {
	...
	mounted() {
		...
		this.tipstart = this.formatDate(this.startObj.time);
		this.tipsend = this.formatDate(this.endObj.time);
		//console.log(utils.isEmptyObject(this.chooseData))
	}
};
</script>
