...
<template>
	<div id="switching">
		...
		<div class="online-box clearfix">
			...
			<el-radio-group v-model="integraloffName" class="fl" style="margin-right: 14px;">
				<el-radio v-for="(item,index) in integralList" :key="index" :label="item.name" border @change.native="clicktheRadioOff(item)"></el-radio>
			</el-radio-group>
		</div>
		<!-- 模版展示 -->
		<div class="online-box clearfix">
		...
		methods: {
			...
			async saveConfig() {
				let data = await http.updateConfig({
					data: {
						type: 1,
						onlineStyle: this.integralOn, //横版风格
						selfhelpStyle: this.integralOff //竖版风格
					}
				});
				if (data) {
			...
			async getWechatConfig() {
				...
				if (data) {
					if (data.onlineStyle) {
						this.integralOn = data.onlineStyle; //在线模版
						this.integralName = this.integralList[this.integralOn - 1].name;
					}
					if (data.selfhelpStyle) {
						this.integralOff = data.selfhelpStyle; //自助模版
						this.integraloffName = this.integralList[this.integralOff - 1].name;
					}
				}
			},
			...
			clicktheRadioOff: function(item) {
				this.integralOff = item.id;
			}
		},
		components: {
			'singleSelect': () =>
