...
<template>
	<section>
		<div id="breakCoupon">
			...
			<div class="right">
				<div class="fl">
					<el-button type="primary" icon="el-icon-plus" @click="openGoodsWindow('1')">选择菜品</el-button>
					<span style="margin-right: 20px;">共关联商品{{getArrLength('selectGoods')}}份，套餐{{getArrLength('selectPackages')}}份</span>
					<span>数量:</span>
					<el-input v-model="goodsNum" placeholder="1～99" maxlength="2" style="width:80px;" onkeyup="value=value.replace(/[^\d]/g,'')"></el-input>
				</div>
				<div class="fl handle-tips">
					<i></i> 购买商品只能关联一个,数量可以定义多个
				</div>
			</div>
			<!-- 赠送商品 -->
			<div class="left ">
			...
			<div class="right">
				<div class="fl">
					<el-button type="primary" icon="el-icon-plus" @click="openGoodsWindow('2')">选择菜品</el-button>
					<span style="margin-right: 20px;">共关联商品{{getArrLength('selectGoodsSec')}}份，套餐{{getArrLength('selectPackagesSec')}}份</span>
					<span>数量:</span>
					<el-input v-model="giveNum" maxlength="2" placeholder="1～99" style="width:80px;" onkeyup="value=value.replace(/[^\d]/g,'')"></el-input>
				</div>
				<div class="fl handle-tips">
					<i></i> 赠送商品可以定义多个,数量可以定义多个
				</div>
			</div>
			<!-- 是否强折 -->
			<div class="left ">
...
export default {
	...
	mounted() {
		...
		if (!utils.isEmptyObject(this.couponDetail)) {
			...
			//赠送商品数量
			this.giveNum = couponDetail.giveNum;
		...
		checkData() {
			...
			if (this.ischain == '3') {
				...
				if (this.selectGoods.length == 0 && this.selectPackages.length == 0) {
					this.valiData('请选择购买商品或套餐');
					return false;
				}
				//验证第二件商品是否选择
				if (this.selectGoodsSec.length == 0 && this.selectPackagesSec.length == 0) {
					this.valiData('请选择赠送商品');
					return false;
				}
				if (this.goodsNum == '' || this.goodsNum < 1 || this.goodsNum > 100) {
					this.valiData('请填写购买商品数量1-100');
					return false;
				}
				if (this.giveNum == '' || this.giveNum < 1 || this.giveNum > 100) {
					this.valiData('请填写赠送商品数量1-100');
					return false;
				}
				//领券多长时间有效的限制
			...
			getSendInfo() {
				...
				if (this.checkData()) {
					...
					//赠送单品
					obj.giveGids = this.arrToString(this.selectGoodsSec);
					//赠送套餐
					obj.givePids = this.arrToString(this.selectPackagesSec);
					//赠送的数量
					obj.giveNum = this.giveNum;
