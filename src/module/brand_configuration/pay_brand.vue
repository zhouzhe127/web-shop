				...
				if(res == 'ok'){
					if(this.isOk(detial)){
						...
						if(detial.paymentName == '支付宝'){
							obj.alipayrsaPublicKey = detial.payConfig.alipayrsaPublicKey;//支付宝公钥
...
//			},
			...
			isOk:function(detial){
				if(detial.paymentName == '微信'){
					...
					if(!global.checkData({appid:'微信APPID不能为空'},detial.payConfig))return false;
					...
					if(!global.checkData({key:'商户支付密钥不能为空'},detial.payConfig))return false;
					// let appSecret = detial.payConfig.appSecret;
					if(!global.checkData({appSecret:'微信appSecret不能为空'},detial.payConfig))return false;
					let apiclient_cert = detial.payConfig.apiclient_cert;
					if(!global.checkData({apiclient_cert:'证书（cert）不能为空'},detial.payConfig))return false;
					if(apiclient_cert !==''){
					...
					if(!global.checkData({apiclient_key:'证书密钥（key）不能为空'},detial.payConfig))return false;
					if(apiclient_key !==''){
						if(splitkey(apiclient_key) != 'BEGIN PRIVATE KEY' || splitkey(apiclient_key) == undefined){
							this.$store.commit('setWin',{
								title:'提示',
								winType:'alert',
					...
					if(!global.checkData({alipayrsaPublicKey:'支付宝公钥不能为空'},detial.payConfig))return false;
					if(!global.checkData({rsaPrivateKey:'RSA私钥不能为空'},detial.payConfig))return false;
					if(!global.checkData({appId:'支付宝应用ID不能为空'},detial.payConfig))return false;
...
<style type="text/css" scoped>
	#brandBase .oCont{padding-left:140px;width: 100%;}
	...
	#brandBase .oCont .Box{width: 360px;height: 42px;cursor: pointer;box-sizing: border-box;}
	#brandBase .oCont .oDe{
		...
		border-radius: 5px;}
	#brandBase .oCont .Box .act{background-color: #28a8e0;color:#fff;}
	#brandBase .oCont .oUl{    
