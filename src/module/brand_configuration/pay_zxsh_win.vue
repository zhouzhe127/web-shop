	...
	<section id="payZXSH">
		...
		<section v-if="tabPosition=='0'">
			...
			<div class="payShowList">
				...
				<el-input-number v-model="detial.sort" :min="1" :max="100"></el-input-number>
			</div>
...
export default {
	...
	async mounted() {
		...
		this.$store.commit('setPageTools', [
			{ name: '返回', className: ['wearhouse', 'create'], fn: this.back }
		]);
	},
	props: {
	...
	methods: {
		...
		async updateRecAccount(item) {
			...
			if (
				item.recOpenBankCode !== '' &&
				!nameReg.test(item.recOpenBankCode)
			) {
				this.$store.commit('setWin', {
