<template>
	<div class="select-store-body">
		<div @click="showSort" class="select-body-btn" ref="selectBtn">
			<input type="text" :placeholder="tips" :value="sortName" readonly="readonly" :style="spanWidth"/>
			<em>
				<i></i>
			</em>
		</div>
		<div class="list select-ban" v-show="sortShow" @click="storeList" ref="listBox" :style="listPosition">
			<em :style="emPosition"></em>
			<ul>
				<li data-type="all" class="all" :class="{active:allSelected}" v-if="!single">全部选择</li>
				<li data-type="single" v-for="(item,index) in sort" :data-id="item.id" :key="index"
	...
	export default {
		...
		methods: {
			...
			init(){
				...
				this.spanWidth = this.width ? "width:"+ (this.width-42)+"px;" : "width:158px;";
			...
			storeList(event){
        		...
        		if(target.tagName.toLocaleLowerCase() != "li") return; //阻止一些不必要的操作
        		...
        		if(btnType == "all"){//点击全部
        			...
        			this.clearEach();
        			//单选状态 清空之前的选择
        			if(this.single) list.forEach((item,index)=>{item.selected = false});
        			list.forEach((item,index)=>{
        				if(item.id == id){
...
<style lang="less" scoped>
	.select-store-body{
		...
		.select-body-btn {
			height: 40px;width: 100%;padding-right: 40px;position: relative;cursor: pointer;
			vertical-align: middle;display: inline-block;border: #b3b3b3 solid 1px;
			em{width: 40px;height: 38px;position: absolute;right: 0;top: 0;border-left: 1px solid #b3b3b3;
				i {height: 10px;width: 10px;position: absolute;top: 50%;left: 50%;margin-top: -5px;margin-left: -5px;
					border-top: 10px solid #b3b3b3;border-left: 5px solid transparent;border-right: 5px solid transparent;box-sizing: border-box;
				}
			}
			input{height: 38px;line-height: 38px;display: block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;
				border: 0;color: #666666;padding: 0 10px;cursor: pointer;
			}
		}
		.list{
			position: absolute;left: 0;top: 50px;width: 500px;background: #333;
				box-shadow: 0 2px 2px #666;z-index: 20;
			em{position: absolute;left: 175px;top: -10px;height: 0;width: 0;
				border-bottom:10px solid #333;border-left:7px solid transparent;border-right:7px solid transparent;}
			ul{
				float: left;overflow: auto;max-height: 300px;width: 100%;padding: 0 20px;margin: 20px 0;
				li{height: 40px;line-height: 40px;padding: 0 15px;float: left;color: #fff;border: 1px solid #fff;margin: 5px;cursor: pointer;
					&.active{background: url(../res/images/sign.png) right 103% no-repeat;border-color: #FF9800;}
					&.all{text-align: center;background: none;}
				}
			}
			.handle{overflow: hidden;float: left;width: 100%;
				span{float: left;color: #fff;width: 50%;height: 40px;line-height: 40px;text-align: center;}
			}
		}
	}
</style>