(function (global, factory) {
    ...
    var config = {
        ...
        // 默认菜单配置
        menus: ['head', 'bold', 'fontSize', 'fontName', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'code', 'undo', 'redo','attach'],
    ...
    /*
        ...
        */
    // 构造函数
    function Image(editor,isFile) {
        this.editor = editor;
        this.isFile = isFile;//是否是自定义的文件
        var imgMenuId = getRandom('w-e-img');
        this.$elem = $('<div class="w-e-menu" id="' + imgMenuId + '"><i class="'+ (isFile ? "edit-attach" :"w-e-icon-image")+'"></i></div>');
    ...
    Image.prototype = {
        ...
        _createInsertPanel: function _createInsertPanel() {
            ...
            // (csv,xls,xlsx,ppt,pptx,word,wordx)
                    ...
                    events: [{
                        ...
                        fn: function fn() {
                            ...
                            if (fileList.length) {
                                ...
                                uploadImg.uploadImg(fileList,isFile);
                            }
        ...
        init: function init() {
            ...
            configMenus.forEach(function (menuKey) {
                ...
                if (MenuConstructor && typeof MenuConstructor === 'function') {
                    // 创建单个菜单
                    _this.menus[menuKey] = new MenuConstructor(editor,menuKey == "attach" ? true :false);
                }
            });
    ...
    UploadImg.prototype = {
        ...
        // 上传图片
        uploadImg: function uploadImg(files,isFile) {
            var _this3 = this;
            ...
            arrForEach(files, function (file) {
                ...
                if (/\.(jpg|jpeg|png|bmp|gif|webp)$/i.test(name) === false && !isFile) {
                    // 后缀名不合法，不是图片
                    errInfo.push('\u3010' + name + '\u3011\u4E0D\u662F\u56FE\u7247');
                    return;
                }
                if (maxSize < size && !isFile) {
                    // 上传图片过大
            ...
            // 抛出验证信息
            if (errInfo.length && !isFile) {
                this._alert('图片验证未通过: \n' + errInfo.join('\n'));
                return;
            }
            if (resultFiles.length > maxLength && !isFile) {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if ((typeof result === 'undefined' ? 'undefined' : _typeof(result)) !== 'object') {
                            try {
                                ...
                                _this3._alert('上传文件失败', '上传文件返回结果错误，返回结果是: ' + result);
                                return;
                            }
                        }
                        if (!hooks.customInsert && result.errno != '0') {
                            ...
                            // 数据错误
                            _this3._alert('上传文件失败', '上传文件返回结果错误，返回结果 errno=' + result.errno);
                        } else {
                            
                            if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                                // 使用者自定义插入方法
                                hooks.customInsert(_this3.insertLinkImg.bind(_this3), result, isFile , editor);
                            } else {
                                // 将图片插入编辑器
