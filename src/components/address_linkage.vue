...
<template>
	<div class="address-linkage">
		<div class="inp-box" ref="addressBtn" @click="showMain">
			<input type="text"  placeholder="请选择地区" :value="inputStr" :style="btnWidth" readonly="readonly"/>
			<em><i></i></em>
		</div>
	...
	export default{
		...
		methods:{
			...
			close(event){//关闭弹框 同时移除document的click事件
				var banDom = this.$refs.addressBtn.getElementsByTagName('*'),
					isBan = true;
			...
			showMain(event){ //文本框点击事件 显示隐藏弹出层
...
<style lang="less" >
	.address-linkage{display: inline-block;position: relative;
		.inp-box{display: inline-block;border: 1px solid #ccc;overflow: hidden;vertical-align: middle;cursor: pointer;
			input{width: 138px;height: 38px;line-height: 38px;padding: 0 10px;border: 0;float: left;cursor: pointer;}
		...
		.main{position: absolute;left: 0;z-index: 10;margin-top: -1px;width: 300px;border: 1px solid #ccc;background: #fff;
			.tab{width: 100%;background: #f0f0f0;border-bottom: 1px solid #ccc;border-bottom: 1px solid #ccc;
				height: 35px;position: relative;
				span{float: left;width: 33.33%;text-align: center;height: 35px;line-height: 35px;color: #333;
					position: absolute;top: 0;left: 0;z-index: 2;
					border-right: 1px solid #ccc;cursor: pointer;
					&:nth-child(2){left: 33.33%;}
					&:nth-child(3){left: 66.66%;border-right: 0;}
				}
				.active{color: #ff9800;background: #fff;}
			}
		}
		.list{padding: 10px 0;max-height: 300px;overflow: auto;
			.item{overflow: hidden;
				.sort{float: left;width: 20%;padding: 10px 0;text-align: center;font-weight: bold;color: #333;}
				ul{float: left;width: 80%;
					li{float: left;text-align: center;padding: 10px 5px;color: #333;cursor: pointer;
						&:hover{text-decoration: underline;}
					}
					li.active{color: #ff9800;}
					li.clear{color: #28a8e0;}
				}
			}
