					...
					<el-tabs v-model="activeName">
						...
						<template v-if="!isSingle">
							<el-tab-pane label="全部" name="first">
								<el-checkbox v-if="(item.storeAreaId == areaList.flag) || areaList.flag == -1" v-for="(item,index) in showShopList" :key="index" v-model="item.selected" :label="item.shopName" border size="medium">
								</el-checkbox>
							</el-tab-pane>
	...
	* */
	...
	methods: {
		...
		init() {
			...
			if (
				showNo &&
				this.areaList.list.length > 0 &&
				this.areaList.list[this.areaList.list.length - 1].id != 0
			) {
